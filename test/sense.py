...
def getFrame(d,v,accelData, accelDataOld,frame):

    global delta_t
	
    tempAccel = ((accelData - accelDataOld) / 2) + accelDataOld
	# displacement
    d1 = v*(delta_t)
    ...
    '''d1 = v*(delta_t / 2)
    d2 = (0.5)*accelData*((delta_t / 2)**2)
    d = d1 + d2

    # velocity
    v1 = v
    v2 = accelData*(delta_t / 2)
    v = v1 + v2
  ...
  def notification_loop( self ):
    ...
    if(calibrate == True):
        ...
        for x in range(1, len(axnew)):
          dx,vx,xframes = getFrame(dx,vx,axnew[x], axnew[x-1],xframes)
          dy,vy,yframes = getFrame(dy,vy,aynew[x], aynew[x-1],yframes)
          dz,vz,zframes = getFrame(dz,vz,aznew[x], aznew[x-1],zframes)
          #xyzframes.append( [xframes[-2],yframes[-2],zframes[-2]] )
