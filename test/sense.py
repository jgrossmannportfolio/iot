  ...
  def dataCallback(self, v):
    ...
    if self.starttimer == 0:
      ...
      if(calibrate == True):
        ...
        (Gxyz, Gmag) = MathUtil.convertData(gx1 + int(float(gyroCal[0])), gy1 + int(float(gyroCal[1])), gz1 + int(float(gyroCal[2])), 1.0/.0175)
        (Axyz, Amag) = MathUtil.convertData(vx1 + int(float(accelCal[0])), vy1 + int(float(accelCal[1])), vz1 + int(float(accelCal[2])), 8192.0/9.80665)
